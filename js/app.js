...
function makeRain()
{
	...
	var lineLength = currentWeather == 'thunder' ? 35 : 14;
	...
	var line = this['innerRainHolder' + (3 - Math.floor(lineWidth))].path('M0,0 0,' + lineLength).attr({
		fill: 'none',
		stroke: currentWeather == 'thunder' ? '#777' : '#0000ff',
	...
	TweenMax.fromTo(line.node, 1, {x: x, y: 0- lineLength}, {delay: Math.random(), y: sizes.card.height, ease: Power2.easeIn, onComplete: onRainEnd, onCompleteParams: [line, lineWidth, x, currentWeather]});
}
...
function tick()
{
	...
	for(var i = 0; i < clouds.length; i++)
	{
		if(currentWeather == 'sun' || currentWeather == 'night')
		{
